...
for ($i = 0; $i < count($lines); $i += 6) {
    ...
    $monkeys[$id]["test"] = $modnumbers[] = preg_replace("(Test: divisible by )", "", $lines[$i + 3]);
...
for ($operations = 0; $operations < 10000; $operations++) {
    for ($monkey = 0; $monkey < count($monkeys); $monkey++) {
        for ($item = 0; $item < count($monkeys[$monkey]["items"]); $item++) {
            ...
            $monkeys[$monkey]["items"][$item] = $monkeys[$monkey]["items"][$item] % array_product($modnumbers);
